...
-- ex:取得所有CLERK的資料後，為了和其他職位的員工對比，現在決定再查詢所有不是
-- 辦事員的員工訊息
SELECT * FROM emp WHERE job<>'CLERK';
SELECT * FROM emp WHERE job!='CLERK';
...
-- ex:找出部門10中所有經理(MANAGER)，部門20中所有辦事員(CLERK)，既不是經理又
-- 不是辦事員但是其餘工資大於或等於2000的所有員工詳細資料，並且要求這些員工
-- 姓名中包含有字母'S'或字母'K'。
SELECT * FROM emp 
...
-- ex:查詢出所有員工訊息，要求按照基本工資由高到低排序，如果工資相等則按照
-- 雇用日期由早到晚進行排序。
