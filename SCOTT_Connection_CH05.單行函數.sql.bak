...
-- ex:查詢員工姓名是'smith'的完整訊息，但是由於失誤，沒有考慮到數據的大小寫問題，此時可
-- 以使用UPPER()函數將全部內容轉為大寫。
...
--                                                                            --
--============================================================================--
-- SYSDATE:取得系統當前日期。默認情況下顯示內容只包含年、月、日，如果想顯示更多
-- 數據，那麼需修改語言環境。ALTER SESSION SET NLS_DATE_FORMAT='yyyy-mm-dd hh24:mi:ss';
-- 重新連線後就恢復默認格式
...
-- ex:顯示每個員工的姓名、工資、職位，同事顯示新的工資(新工資的標準為:辦事員增長10%、
...
-- ➤COALESCE(表達式1,表達式2,表達式3,...表達式n):對null進行操作，採用依次判斷表達式的
-- 方式完成，如果表達式1為null，則顯示表達式2的內容，如果表達式2為null，則顯示表達式3的
-- 內容，依次類推，判斷到最後如果還是null，則最終的顯示結果就是null。
