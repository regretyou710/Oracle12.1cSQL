...
-- ex:查詢出每個員工姓名、職位、基本工資、部門名稱、工資所在公司的員工等級
-- 分析:
-- 1.確定所需要的資料表:
...
--                                                                            --
--============================================================================--
-- ex:增加一筆沒有部門編號的員工姓名為張三豐的紀錄，然後後進行多表關聯查詢
SELECT * FROM emp e, dept d WHERE e.deptno=d.deptno;
-- 查詢結果出現兩個問題:
-- 1.沒有部門的員工沒有顯示
-- 2.有一個40部門沒有顯示
-- 此時可以發現，使用內連接只有滿足連接條件的數據才會全部顯示。
-- 可是如果現在希望emp或dept表中的數據顯示完整，那麼就可以利用外連接進行，而
-- 現在外連接主要使用兩種:左外連接、右外連接。
...
-- 使用外連接的環境:如果所需要的數據訊息沒有顯示出來，那麼就使用外連接，而
-- 具體是左外連接還是右外連接，透過測試後結果進行判斷即可。
--============================================================================--
--                                                                            --
/* ※多表查詢-自身關聯                                                        */
--                                                                            --
...
-- ex:查詢在1981年雇用的全部員工的編號、姓名、雇用日期(按照年-月-日顯示)、工作、
-- 主管姓名、員工月薪資、員工年薪資(基本工資+獎金)、員工工資等級、部門編號、部門
-- 名稱、部門位置，並且要求這些員工的月基本工資在1500~3500之間，將最後的結果按照
-- 年工資的降序排列，如果年工資相等，則按照工作進行排序
-- 分析:
-- 確定所需要的資料表:
...
--============================================================================--
-- ➤交查連接:主要功能就是產生笛卡爾積
-- 一般而言，在進行多表連接的時候都一定會存在關聯欄位以消除笛卡爾積，而關聯
-- 欄位的名稱一般都會一樣，如果不一樣也會有部分相同。
...
-- ➤USING子句:設置連接欄位。透過自然連接可以直接使用關聯欄位消除笛可爾積，
-- 那如果現在的兩張表中沒有存在這種關聯欄位的話，就可以透過USING子句完成
-- 笛卡爾積的消除。
