...
-- 可是如果現在希望emp或dept表中的數據顯示完整，那麼就可以利用外連接進行，而現在
-- 外連接主要使用兩種:左外連接、右外連接。
...
-- 使用外連接的環境:如果所需要的數據訊息沒有顯示出來，那麼就使用外連接，而具體是左
-- 外連接還是右外連接，透過測試後結果進行判斷即可。
--============================================================================--
--                                                                            --
/* ※多表查詢-自身關聯                                                        */
--                                                                            --
...
--============================================================================--
-- ➤交查連接:主要功能就是產生笛卡爾積
-- 一般而言，在進行多表連接的時候都一定會存在關聯欄位以消除笛卡爾積，而關聯欄位的
-- 名稱一般都會一樣，如果不一樣也會有部分相同。
...
-- ➤USING子句:設置連接欄位。透過自然連接可以直接使用關聯欄位消除笛可爾積，那如果
-- 現在的兩張表中沒有存在這種關聯欄位的話，就可以透過USING子句完稱笛卡爾積的消除。
