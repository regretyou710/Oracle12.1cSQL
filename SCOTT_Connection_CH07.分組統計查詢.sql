...
--   ③使用COUNT(*)最為方便，但建議使用COUNT(欄位)，使一個不可能為null的列進
--     行統計。例如:主鍵。
...
-- ▲注意事項一:
-- 如果在一個查詢之中不存在GROUP BY子句，那麼在SELECT子句之中只允許出現
-- 統計函數，其他任何的欄位都不允許出現。
-- 錯誤程序:
...
-- ▲注意事項二:
-- 在統計查詢之中(存在了GROUP BY子句)，SELECT子句之中只允許出現分組欄位
-- (GROUP BY之後定義的欄位)和統計函數，其他的任何欄位都不允許出現。故，在進行
-- 分組操作的時候，遵守一個原則:GROUP BY子句之中允許出現的欄位才是在SELECT子句
-- 之中允許出現的欄位。
-- 錯誤程序:
SELECT deptno, ename, COUNT(empno) FROM emp GROUP BY deptno;
-- 正確程序:
SELECT deptno, COUNT(empno) FROM emp GROUP BY deptno;
-- ▲注意事項三:所有統計函數允許嵌套使用。但是一旦使用了嵌套的統計函數之後，
-- SELECT子句之中不允許再出現任何的欄位，包括分組欄位。
-- ex:求出每個部門平均工資最高的工資
-- 此時因為SELECT子句之中存在了deptno的欄位，所以出現錯誤(ORA-00937: 不是單一
-- 群組的群組函數)。
...
-- 步驟二:觀察現在的dname欄位(查詢結果，理解為臨時表)，既然重複的列就可以
-- 進行分組。
-- 針對臨時表數據進行分組。
SELECT 
d.dname, 
...
-- 步驟三:針對部門表是包含了四個部門訊息，可是此處只顯示三個部門訊息，需要
-- 加入外連接顯示40部門的統計。
SELECT 
d.dname, 
COUNT(e.empno), 
...
GROUP BY e.comm
;
-- 查詢結果會發現，並未顯示出佣金的領與不領的分組，而是變成按照每一種可能出現
-- 的佣金數值來進行分組。因此，需更換查詢過程。首先先查詢出所有領取佣金的員工
-- 訊息，再查出不領取佣金的員工訊息，這兩者查詢結果的返回列形式完全相同則可使
-- 用UNION語句。
SELECT 
'領取佣金' 領用類型 , 
COUNT(e.empno) 員工人數 , 
...
FROM emp e 
;
-- 無法使用WHERE進行條件過濾，因為WHERE子句執行優先於GROUP BY子句，而且不允許
-- 使用統計函數
SELECT 
e.job, 
ROUND(AVG(e.sal),2) 平均工資, 
...
-- ex:列出至少有一個員工的所有部門編號、名稱，並統計出這些部門的平均工資、
-- 最低工資、最高工資
...
-- ex:顯示非銷售人員工作名稱以及從事同一工作員工的月工資的總和，並且要滿足
-- 從事同一工作的員工的月工資合計大於5000，輸出結果按月工資的合計升序排列。
-- 步驟一:顯示非銷售人員工作名稱
