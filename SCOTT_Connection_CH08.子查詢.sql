...
-- 	= 560000條紀錄。
-- 方法二:子查詢
-- 	統計:emp表的1400條紀錄，而且最終的統計結果的行數不可能超過400行(在步驟二透過部門編
-- 	號分類統計，而分類只有4種*100倍)
...
--                                                                            --
--============================================================================--
-- 簡述:子查詢可以出現任意位置上，不過從實際的項目來講，在WHERE、FROM、HAVING子句
-- 之中使用子查詢的情況還是比較多的，而對於SELECT子句，只能是以一種介紹的形式進行說明。
...
FROM dept d;
-- 由上兩者執行後顯示，分組與不分組返回結果相同，原因是執行SELECT子句時已經先取得
-- 當前部門的訊息。
--============================================================================--
--                                                                            --
/* ※子查詢-WITH子句                                                          */
--                                                                            --
--============================================================================--
-- 簡述:可以使用WITH子句創建臨時查詢表。臨時表實際上就是一個查詢結果，如果一個查詢
-- 結果返回是多行多列，那麼就可以將其定義在FROM子句之中，表示其為一張臨時表。除了在
-- FROM子句之中出現臨時表之外，也可以利用WITH子句直接定義臨時表，就可以繞開FROM子句。
...
-- ex:查詢每個部門工資最高的員工編號、姓名、職位、雇用日期、工資、部門編號、部門名稱
-- ，顯示的結果按照部門編號進行排序。
WITH e AS(
SELECT 
...
題意為設置2行物理偏移，所以在進行部門區分後的sum要從分區後的第三列開始比較，前兩列的
sum都是累加而來。
...
-- ➤分析函數應用-報表函數:
-- ex:CUME_DIST()函數操作
-- 說明:CUME_DIST()函數他是計算相對位置，例如:假設分區有5條紀錄，那麼這些紀錄會按照1、
...
FROM emp 
GROUP BY deptno;
-- 以上方式使用的是DECODE()函數，但是對於這個函數屬於Oracle自己的色，如果沒有此函數
-- 只能夠利用SELECT子句使用子查詢的方式完成。
