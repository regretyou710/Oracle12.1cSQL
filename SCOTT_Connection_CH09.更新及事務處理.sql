...
-- 如果真的執行了這樣的操作，假設現在數據表中有500萬條紀錄，那麼如果按照每一條更
-- 新的時間為0.01s，那麼這500萬條紀錄總體的更新時間是50000，5萬秒=13小時，那麼就
-- 意味著這13個小時之內，所有的數據都無法被其他用戶修改，所以這種更新全部的操作
-- 是不可能出現的。但現實工作中出現了此類問題並非沒有辦法解決，對於軟件問題的解決
-- ，實際上就只有兩句話:時間換空間、空間換時間。
...
--                                                                            --
--============================================================================--
-- 簡述:在刪除數據時如果沒有指定刪除條件，那麼就表示刪除全部數據，而對於刪除
-- 條件，用戶也可以直接編寫子查詢完成。
...
/*
-- ▲note:
對於更新的三個操作:增加、修改、刪除，每一次都一定會返回當前操作所影響到的數據
行數，在JAVA的JDBC操作中更新數據的操作Statement和PreparedStatement兩個介面，調
用的方法是executeUpdate()，返回的是一個int行數據，就是接收更新的行數。如果沒有
數據被更新，一定返回的是0行。
*/
--============================================================================--
--                                                                            --
...
--                                                                            --
--============================================================================--
-- 說明:事務處理在數據庫開發中有著非常重要的作用，所謂的事務核心概念就是指
-- 一個SESSION所進行的所有更新操作要不一起成功，要不一起失敗，事務本身具有:
-- ①原子性(Atomicity)
-- ②一致性(Consistency)
-- ③隔離性或獨立性(Isolation)
-- ④持久性(Durabilily)
...
-- ➤SESSION:指的是會話，每一個連接到服務器上的用戶都透過SESSION表示，服務器
-- 依靠SESSION來區分不同的用戶，所以在所有的開發中，會話都表示用戶。
...
/*
所有的命令，每一個SESSION全都具備。為了更好的觀察出事務的特點，所以下面會使用
多個sqlplus窗口進行功能的展示。
*/
conn c##scott/tiger;
set linesize 300;
...
-- ➤更新緩衝
-- 說明:對於每一個SESSION而言，每一個數據庫的更新操作在事務沒有被提交之前都
-- 只是暫時保存在了一段緩衝區之中，並不會真正的向數據庫中發出命令，如果現在
-- 用戶發現操作有問題了，則可以進行事務的回滾。
...
-- ➤回滾存儲點
-- 說明:在默認情況下，執行ROLLBACK命令意味著全部的操作都要回滾，如果現在希望
-- 可以回滾到指定操作的話，則可以採用SVAEPOINT設置一些保存點，這樣在回滾的時
-- 候就可以透過ROLLBACK返回指定的保存點上。
-- note:在沒有執行COMMIT指令前，卻執行了ROLLBACK回滾操作，那麼會回到原點上。
-- 所以為了操作方便，提供了保存點。
...
SELECT * FROM myemp WHERE deptno=10 FOR UPDATE;
-- 在第二個SESSION窗口會發現被鎖住不會動作，因為所有的數據只能夠被一個SESSION
-- 操作。
-- 在第一個SESSION窗口執行ROLLBACK解鎖後，第二個SESSION窗口便可執行語句


-- ➤行級鎖定
-- 說明:用戶執行了INSERT、UPDATE、DELETE以及SELECT FOR UPDATE語句時，Oracle將
-- 隱式的實現紀錄的鎖定，這種鎖定被稱為排他鎖。行級鎖定主要特點是:當一個事務
-- 執行了相應的數據操作之後，如果此時事務沒有提交，那麼會一直以獨占的方式鎖定
-- 這些操作的數據，其他事務一直到此事務釋放鎖後才可以進行操作。
...
-- 此時更新的紀錄和第一個SESSION是相同的，而且第一個SESSION沒有提交事務。如果
-- 更新了同一條紀錄，那麼第二的SESSION也會出現鎖的情況。
...
NOWAIT:這是一個可選項，當視圖鎖定一張數據表時，如果發現已經被其他事務鎖定，就
不會等待。鎖定模式有如下幾種常見模式:
①ROW SHARE:行共享鎖，在鎖定期間允許其他事務併發對表進行各種操作，但不允許任何
事務對同一張表進行獨占操作(禁止排他鎖)。
②ROW EXCLUSIVE:行排他鎖，允許用戶進行任何操作，與行共享鎖不同的是不能防止別的
事務對同一張表進行手動鎖定或獨占操作。
③SHARE:共享鎖，其他事務指允許執行查詢操作，不能執行修改操作。
④SHARE ROW EXCLUSIVE:共享排他鎖，允許任何用戶進行查詢操作，但不允許其它用戶使
用共享鎖，之前所用的"SELECT FOR UPDATE"就是共享排他鎖的常見應用。
⑤EXCLUSIVE:排他鎖，事務將以獨占方式鎖定表，其他用戶允許查詢，但是不能修改，也
不能設置任何的鎖。            
*/
...
-- ➤解除鎖定
-- 說明:盡管用戶清楚了鎖產生的原因，但是在很多的時候由於業務量增加，並不會明
-- 確的列出鎖的種種可能，所以此時就必須透過其他方式查看是否出現了鎖定以及透過
-- 命令手動的解除鎖定。
/*
解除鎖定語法:
ALTER SYSTEM KILL SESSION 'SID,SERIAL#';
在此格式之中發現如果要想結束一個SESSION(結束一個SESSION就表示解鎖)，則需要
兩個標記:SESSION ID(SID)、另一個就是序列號(SERIAL#)，而這兩個內容可以利用
"v$locked_object"和"v$session"兩個數據字典查詢得到。
*/
...
-- 此處就出現了一個SESSION ID(每一個用戶的SESSION由管理員分配)。但是只知道
-- SESSION ID還是無法解除鎖定，所以還必須查看v$session數據字典。
