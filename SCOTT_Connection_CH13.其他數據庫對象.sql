...
SELECT * FROM v_emp20;
-- note:在進行視圖增加數據的時候，如果視圖只包含表中的部分數據，那麼增加時也只會
-- 增加部分數據，沒有映射到試圖之中的直行內容，都會使用null填充。
...
-- ➤WITH CHECK OPTION子句
-- 說明:在創建視圖時有時候需要使用一些WHERE子句作一些條件的限制，但是默認情況
-- 下的視圖創建完成後，是可以透過視圖去修改在WHERE子句之中所使用的欄位內容的
-- ，而在此時就需要透過WITH CHECK OPTION子句來保證視圖的創建條件不被更新。
-- 語法
...
select * from v_emp20;
select * from emp;
-- 發現修改視圖之後，原始數據被改變了，但現在的問題又回來了，對於此時的操作
-- ，deptno欄位是視圖存在的核心條件，把核心條件修改了，合適嗎?
...
-- 現在使用WITH CHECK OPTION子句可以保證創建欄位不被修改，但如果現在要修改的
-- 是其他欄位又會如何?
-- ex:修改v_emp20視圖中員工編號7369的訊息
UPDATE v_emp20 SET ename='史密斯',comm=300 WHERE empno=7369;
select * from v_emp20;
ROLLBACK;
-- 發現除了deptno欄位之外，其他的欄位依然可以更新。但是視圖本身不是真實數據，
-- 它屬於映射數據，應該在基表之中更新才合適。
...
-- 化。
-- 對於序列之中緩存是一個非常重要的概念，在序列使用之前，已經在內存裡面偽用戶提
-- 供好了一系列的生成序列號。用的時候不是隨用隨取，而是已經準備好了。
-- 如果設置了緩存，如果數據庫出現了問題，那麼可能這些緩存的數據就會消失，就會出
-- 現跳號的問題。
...
-- ➤自動序列(Oracle 12c新特性)
-- 說明:從Oracle 12c起，為了方便用戶生成數據表的流水編號，所以提供了類似於
-- DB2或MySQL那樣的自動增長列，而這種自動增長列實際上也是一個序列，只是序列
-- 對象的定義是由Oracle數據庫自己控制的。
/*
語法:
CREATE TABLE 表名稱(
...
--                                                                            --
--============================================================================--
-- 說明:主要是進行查詢優化。在數據庫之中，索引是一種專門用於數據庫查詢操作性能的
-- 一種手段。在Oracle中為了維護這種查詢性能，需要對某一類數據進行指定結構的排列。
-- 但是在Oracle之中，針對於不同的情況會有不同的索引使用。
...
-- B-Tree索引由分支塊(branch block)和葉塊(leaf block)組成。在樹結構中，位於最底
-- 層底塊被稱為葉塊，包含每個被索引列的值和行所對應的ROWID。在葉節點的上面是分支
-- 塊，用來導航結構，包含了索引列(關鍵字)範圍和另一索引塊的地址。
-- 主要包含的組件如下:
