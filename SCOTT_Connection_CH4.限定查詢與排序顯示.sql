...
-- ex:統計出基本工資高於1500的全部雇員訊息
SELECT * FROM EMP WHERE SAL>1500;


-- ex:要求查詢出所有基本工資小於等於2000的全部員工訊息
SELECT * FROM emp WHERE sal<=2000;


-- ex:根據之前的查詢結果發現SMITH的工資最低，所以現在希望可以取得SMITH的詳細資料
SELECT * FROM emp WHERE ename='SMITH';


-- ex:查詢出所有辦事員(CLERK)的員工訊息
SELECT * FROM emp WHERE job='CLERK';


-- ex:取得所有CLERK的資料後，為了和其他職位的員工對比，現在決定再查詢所有不是辦事員的員工訊息
SELECT * FROM emp WHERE job<>'CLERK';
SELECT * FROM emp WHERE job!='CLERK';


-- ex:查詢出工資範圍再1500~3000(包含1500和3000)的全部員工訊息
SELECT * FROM emp WHERE sal>=1500 AND sal<=3000;


-- ex:查詢職位是銷售人員，並且基本工資高於1200的所有員工訊息
SELECT * FROM emp WHERE job='SALESMAN' AND sal>1200;


-- ex:要求查詢出10部門中的經理或是20部門的業務員訊息
SELECT * FROM emp WHERE (deptno=10 AND job='MANAGER') OR (deptno=20 AND job='CLERK');


-- ex:查詢不是辦事員且基本工資大於2000的全部員工訊息
...
-- ex:查詢員工編號是7369、7788、7566之外的員工訊息
SELECT * FROM emp WHERE empno NOT IN(7369,7788,7566);


-- ex:在IN範圍中指定含有null時，不影響返回結果
SELECT * FROM emp WHERE mgr IN(7369,7788,null);


-- ex:在NOT IN範圍中指定含有null時，返回無資料的結果
SELECT * FROM emp WHERE mgr NOT IN(7369,7788,null);
--============================================================================--
--                                                                            --
/* ※限定查詢-模糊查詢                                                          */
--                                                                            --
...
-- ➤LIKE、NOT LIKE。須注意:若查詢內容不設置關鍵字，則返回查詢全部結果。
-- ex:查詢出員工姓名是以S開頭的全部員工訊息
SELECT * FROM emp WHERE ename LIKE 'S%';


-- ex:要求查詢員工姓名的第二個字母是'M'的全部員工訊息
SELECT * FROM emp WHERE ename LIKE '_M%';


-- ex:查詢出姓名中任意位置包含字母'F'的員工訊息
SELECT * FROM emp WHERE ename LIKE '%F%';


-- ex:查詢員工姓名長度為6或超過6的員工訊息
SELECT * FROM emp WHERE ename LIKE '______%';


-- ex:查詢出員工基本工資中包含1或者是在81年雇佣的全部員工訊息
SELECT * FROM emp WHERE sal LIKE '%1%' OR hiredate LIKE '%81%';


-- ex:找出部門10中所有經理(MANAGER)，部門20中所有辦事員(CLERK)，既不是經理又不是辦事員
-- 但是其餘工資大於或等於2000的所有員工詳細資料，並且要求這些員工姓名中包含有字母'S'或字母'K'。
SELECT * FROM emp 
...
-- ex:查詢員工的完整訊息並按照基本工資由低到高排序
SELECT * FROM emp ORDER BY sal;
SELECT * FROM emp ORDER BY sal ASC;


-- ex:查詢出所有辦事員(CLERK)的詳細資料，並按照基本工資由低到高排序
SELECT * FROM emp WHERE job='CLERK' ORDER BY sal;


-- ex:查詢出所有員工訊息，要求按照基本工資由高到低排序，如果工資相等則按照雇用日期由早到晚進行排序。
