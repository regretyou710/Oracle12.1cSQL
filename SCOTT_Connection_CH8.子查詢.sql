...
-- ex:查詢出所有在"SALES"(銷售部)工作的員工的編號、姓名、基本工資、獎金、職位、雇用日期
-- 、部門的最高和最低工資
-- 步驟一:找出銷售部的編號
...
-- ex:查詢出所有工資高於公司平均工資的員工編號、姓名、基本工資、職位、雇用日期、所在
-- 部門名稱、位置、主管姓名、公司的工資等級、部門人數、平均工資、平均服務年限。
-- 確定所需要的表:
...
--                                                                            --
--============================================================================--
-- 簡述:子查詢可以出現任意位置上，不過從實際的項目來講，在WHERE、FROM、HAVING子句之中使用
-- 子查詢的情況還是比較多的，而對於SELECT子句，只能是以一種介紹的形式進行說明。
...
FROM dept d;
-- 由上兩者執行後顯示，分組與不分組返回結果相同，原因是執行SELECT子句時已經先取得當前部門
-- 的訊息。
--============================================================================--
--                                                                            --
/* ※子查詢-WITH子句                                                           */
--                                                                            --
--============================================================================--
-- 簡述:可以使用WITH子句創建臨時查詢表。臨時表實際上就是一個查詢結果，如果一個查詢結果返回
-- 是多行多列，那麼就可以將其定義在FROM子句之中，表示其為一張臨時表。除了在FROM子句之中出
-- 現臨時表之外，也可以利用WITH子句直接定義臨時表，就可以繞開FROM子句。
...
-- ex:查詢每個部門工資最高的員工編號、姓名、職位、雇用日期、工資、部門編號、部門名稱，顯示
-- 的結果按照部門編號進行排序。
WITH e AS(
SELECT 
...
--                                                                            --
--============================================================================--
-- 簡述:傳統SQL就是SQL標準規定的語法，SELECT、FROM、WHERE、GROUP BY、HAVING、ORDER BY
-- ，但是傳統SQL所能完成的功能實際上不多。
...
-- PARTITION BY子句:將一個簡單的結果集分為N組(或稱為分區)，而後按照不同的組對數據進行統
-- 計。
-- NULLS FIRST|NULLS LAST:表示返回數據行中包含NULL值是出現在排序序列前還是尾。
-- WINDOWING子句(代名詞):給定在定義變化的固定的數據窗口方法，分析函數將對此數據進行操作。
-- ORDER BY子句:主要就是進行排序，但是現在實現的是分區內數據的排序，而這個排序會直接影響到
-- 最終的查詢結果。
-- ORDER BY子句選項:
--    NULLS FIRST:表示在進行排序前，出現null值的數據行排列在最前面。
--    NULLS LAST:表示出現的null值數據行排列在最後面。

-- 在分析函數之中存在有三種子句:PARTITION BY、ORDER BY、WINDOWING，而這三種子句的組合
-- 順序有如下幾種
...
-- 這個時候只是一個簡單查詢，但是在這個SELECT子句裡面是不可能出現統計函數，如:COUNT()、
-- SUM()，因為統計函數1.單獨使用 2.與GROUP BY一起使用。但有了分析函數的支持後:
SELECT deptno, ename, 
...
-- ➤WINDOWING子句
-- 分窗子句主要是用於定義一個變化或固定的數據窗口方法，主要用於定義分析函數在操作行的集
-- 合，分窗子具有兩種實現方式:
-- 	實現一:值域窗(RANGE WINDOW)，邏輯偏移。當前分區之中當前行的前N行到當前行的紀錄集。
-- 	實現二:行窗(ROWS WINDOW)，物理偏移。以排序的結果順序計算偏移當前行的起始行紀錄集。
...
-- 	FOLLOWING:如果不寫此語句表示使用上N行與當前行指定數據比較，如果編寫此語句，表示當前
-- 			      行與下N行數據比較。
...
題意為設置2行物理偏移，所以在進行部門區分後的sum要從分區後的第三列開始比較，前兩列的sum都
是累加而來。
...
-- ➤分析函數應用-數據統計函數:
-- ex:查詢員工編號是7369的員工姓名、職位、基本工資、部門編號、部門的人數、部門平均工資、
-- 部門最高工資、部門最低工資、部門總工資。
...
-- ex:查詢員工的編號、姓名、基本工資、所在部門名稱、部門位置即此部門的平均工資、最高工資
-- 、最低工資。
-- 確定所需資料表:
-- emp表:員工的編號、姓名、基本工資
-- dept表:部門名稱、部門位置
...
FROM emp;
-- 由以上操作顯示，針對於所有的數據，ROW_NUMBER()自動生成行號，但是在每一個分區裡面它也
-- 存在。


-- ex:KEEP語句操作;查詢每個部門的最高及最低工資
-- 說明:KEEP語句的功能是保留滿足條件的數據，而且在使用DENSE_RANK()函數確定的集合後才可以
-- 使用，透過FIRST或LAST取得集合中的數據。
...
-- 說明:OVER()是聲明一個數據集合，而利用FIRST_VALUE()或LAST_VALUE()函數取得集合中首行
-- 或尾行。
...
-- ➤分析函數應用-報表函數:
-- ex:CUME_DIST()函數操作
-- 說明:CUME_DIST()函數他是計算相對位置，例如:假設分區有5條紀錄，那麼這些紀錄會按照 1、
-- 0.8、0.6、0.4、0.2的方式進行劃分。
SELECT deptno, ename, sal, 
...
FROM emp 
GROUP BY deptno;
-- 以上方式使用的是DECODE()函數，但是對於這個函數屬於Oracle自己的色，如果沒有此函數只能夠
-- 利用SELECT子句使用子查詢的方式完成。
...
-- ex:在PIVOT函數使用上有多欄位分組需求，增加一個sex欄位同時更新81年僱用的雇員的性別為女
...
--============================================================================--
-- ➤層次函數
-- 簡述:層次查詢是一種較為確定數據行之間關係結構的一種操作。如:學校分為"教學管理層"、
-- "教師層"、"學生層"這樣三層次結構。
/*
語法結構:
LEVEL...
...
語法組成:
	...
	CONNECT BY:指的是數據之間的連接，如:員工數據依靠mgr找到其主管，就是一個連接條件，
			   其中NOCYCLE需要結合CONNECT_BY_ISCYCLE偽列確定出父子節點循環關係。
...
-- "CONNECT_BY_ISLEAF"偽列判斷某一個節點是根節點還是葉子節點，如果直行返回的是數字0，
-- 則表示根節點，反之返回為1。
-- ex:判斷是否為子節點
SELECT empno,LPAD('|-',LEVEL*2,' ') || ename, mgr, LEVEL, 
...
-- ➤CONNECT_BY_ROOT列
-- 說明:CONNECT_BY_ROOT主要作用是取得某一個欄位在本次分層之中的根節點數據名稱，例如:
-- 如果按照主管層次劃分，則所有數據的根節點都應該是KING。
-- ex:
SELECT empno,LPAD('|-',LEVEL*2,' ') || ename, mgr, LEVEL, 
...
-- 說明:利用"SYS_CONNECT_BY_PATH()"函數按照給出的節點關係，自動的將前根節點中的所有相關
-- 路徑進行顯示。
...
-- 說明:在使用層次查詢進行數據顯示時，如果用戶直接使用ORDER BY子句進行指定欄位的排序
-- ，有可能會破壞數據的組成結構。
-- ex:直接使用ORDER BY排序
SELECT empno,LPAD('|-',LEVEL*2,' ') || ename, mgr, LEVEL, 
DECODE(CONNECT_BY_ISLEAF,0,'根節點',1,'    子節點') ISLEAF 
...
-- 說明:在進行數據層次設計的過程之中，最為重要根據指定的數據列確定數據間的層次關係，但是
-- 有時候也可能出現死循環，如:KING的主管是BLAKE，而BLAKE的領導也是KING就表示一個循環關係
-- ，為了判斷循環關係的出現，在Oralce中也提供一個CONNECT_BY_ISCYCLE偽列，來判斷是否會出
-- 現循環，如果出現循環，則顯示1，沒有出現循環，則顯示0。同時如果想要判斷是否為循環節點，
-- 則還需要"NOCYCLE"的支持。
