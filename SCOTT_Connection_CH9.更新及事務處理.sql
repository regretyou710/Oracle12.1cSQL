...
--                                                                            --
--============================================================================--
-- 簡述:在刪除數據時如果沒有指定刪除條件，那麼就表示刪除全部數據，而對於刪除條件，用戶也
-- 可以直接編寫子查詢完成。
...
--                                                                            --
--============================================================================--
-- 說明:事務處理在數據庫開發中有著非常重要的作用，所謂的事務核心概念就是指一個SESSION所進
-- 行的所有更新操作要不一起成功，要不一起失敗，事務本身具有:
-- ①原子性(Atomicity)
-- ②一致性(Consistency)
-- ③隔離性或獨立性(Isolation)
-- ④持久性(Durabilily)
...
-- ➤SESSION:指的是會話，每一個連接到服務器上的用戶都透過SESSION表示，服務器依靠SESSION
-- 來區分不同的用戶，所以在所有的開發中，會話都表示用戶。
...
/*
所有的命令，每一個SESSION全都具備。為了更好的觀察出事務的特點，所以下面會使用多個sqlplus
窗口進行功能的展示。
*/
conn c##scott/tiger;
set linesize 300;
...
-- ➤更新緩衝
-- 說明:對於每一個SESSION而言，每一個數據庫的更新操作在事務沒有被提交之前都只是暫時保存在
-- 了一段緩衝區之中，並不會真正的向數據庫中發出命令，如果現在用戶發現操作有問題了，則可以進
-- 行事務的回滾。
...
-- ➤回滾存儲點
-- 說明:在默認情況下，執行ROLLBACK命令意味著全部的操作都要回滾，如果現在希望可以回滾到指定
-- 操作的話，則可以採用SVAEPOINT設置一些保存點，這樣在回滾的時候就可以透過ROLLBACK返回指
-- 定的保存點上。
-- note:在沒有執行COMMIT指令前，卻執行了ROLLBACK回滾操作，那麼會回到原點上。所以為了操作
-- 方便，提供了保存點。
...
-- 實現紀錄的鎖定，這種鎖定被稱為排他鎖。行級鎖定主要特點是:當一個事務執行了相應的數據操作
-- 之後，如果此時事務沒有提交，那麼會一直以獨占的方式鎖定這些操作的數據，其他事務一直到此事
-- 務釋放鎖後才可以進行操作。
...
-- 此時更新的紀錄和第一個SESSION是相同的，而且第一個SESSION沒有提交事務。如果更新了同一條
-- 紀錄，那麼第二的SESSION也會出現鎖的情況。
...
-- ➤解除鎖定
-- 說明:盡管用戶清楚了鎖產生的原因，但是在很多的時候由於業務量增加，並不會明確的列出鎖的
-- 種種可能，所以此時就必須透過其他方式查看是否出現了鎖定以及透過命令手動的解除鎖定。
/*
解除鎖定語法:
